...
if matches[1] == 'delbye' and not matches[2] and is_sudo(msg) then 
    
   redis:hdel(hash,'welcome')
        return 'group bye has been deleted🗑'
end
...
if matches[1] == 'setbye' and is_sudo(msg) then
redis:hset(hash,'bye',matches[2])
        return 'group bye has been setted to : ✋\n'..matches[2]
end
